const {HCVViral} = require('../model/hcvViral')
...
{
    const hematologyData =await HCVViral.find();
...
router.get('/:RequestID',async(req,res)=>
{
  ...
  console.log(req.params.RequestID); 
    const hematologyData =await HCVViral.findOne({RequestID:Number(req.params.RequestID)});
    console.log(hematologyData)
    res.send(hematologyData);
})
router.put('/:RequestID',async(req,res)=>{
  console.log(req.params.RequestID)
    ...
    var hematology = await HCVViral.update({RequestID:req.params.RequestID},{
      $set:{
        status:"completed",
...
const generateUHID=async()=>
{
    ...
    let user = await HCVViral.findOne({RequestID:r})
    if(!user) return r
    else  return (r + 1)
